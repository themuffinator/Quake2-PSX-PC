...
- The word LOADING is WHITE, and the front end's `STARTING` / `GAME` is blue. `0x80079398` sets drawable 0's `+0x48` — the highlight flag — immediately after installing the LOADING record, and highlight at the 16-pixel size selects palette 70, the white-only mask, over 68's blue ramp. The front end's own page is built by the ordinary page builder and sets nothing, so it keeps the ramp. The page is pure text and no cursor sits on it, so the flag cannot come from the selection: `q2_menu_draw_opts` carries it explicitly now.
